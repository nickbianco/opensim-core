    ...
    for (const auto& force : forces) {
        if (force.hasProperty("path")) {
            ...
            const AbstractGeometryPath& path =
                    force.getPropertyByName<AbstractGeometryPath>("path")
                            .getValue();
            auto coordinatePaths = path.findIndependentCoordinatePaths(state);
            ...
            momentArmMap[forcePath] = std::move(coordinatePaths);
        }
    }
...
void PolynomialPathFitter::evaluateFunctionBasedPaths(Model model,
    ...
    // Create a moment arm map based on the fitted model paths. The name of
    // each FunctionBasedPath is the absolute path of the force it replaced.
    MomentArmMap momentArmMap;
