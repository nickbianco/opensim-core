...
# Load the states
states1 = osim.TimeSeriesTable(os.path.join(build_path, "states_0_zero_control0.25_path0.sto"))
states2 = osim.TimeSeriesTable(os.path.join(build_path, "states_1_zero_control0.25_path1.sto"))
...
for i in range(side):
    for j in range(side):
        ...
        axs[i, j].plot(time1, states1.getDependentColumn(labels1[i*side + j + 1]))
        axs[i, j].plot(time2, states2.getDependentColumn(labels2[i*side + j + 1]))
plt.show()
