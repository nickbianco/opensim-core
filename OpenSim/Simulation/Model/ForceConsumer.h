...
public:
    ...
    /**
     * Consumes a vector of body forces.
     *
     * @param state    the state that was used to evaluate the force
     * @param force    the vector of body forces, specified in the inertial
     *                 (ground) frame
     */
    void consumeBodyForces(
        const SimTK::State& state,
        const SimTK::Vector_<SimTK::SpatialVec>& bodyForces)
    {
        implConsumeBodyForces(state, bodyForces);
    }
...
private:
    ...
    */
    ...
    /**
     * Subclasses of `ForceConsumer` may implement this method. There are no
     * expectations on how an implementation should handle a call to this
     * function.
     *
     * @param state       the state that was used to evaluate the force
     * @param bodyForces  the vector of body forces, specified in the inertial
     *                    (ground) frame
     */
    virtual void implConsumeBodyForces(
        const SimTK::State& state,
        const SimTK::Vector_<SimTK::SpatialVec>& bodyForces)
    {}
};
