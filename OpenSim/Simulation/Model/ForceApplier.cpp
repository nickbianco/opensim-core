...
void OpenSim::ForceApplier::implConsumeBodyForces(
    const SimTK::State& state,
    const SimTK::Vector_<SimTK::SpatialVec>& bodyForces)
{
    OPENSIM_ASSERT_ALWAYS(bodyForces.size() == _bodyForces->size() &&
            "the provided body forces vector is the wrong size");
