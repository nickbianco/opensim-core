...
void Scholz2015GeometryPath::produceForces(const SimTK::State& state,
    ...
    for (const auto& [ix, eltIx] : _viaPointIndexes) {
        ...
        // via point torwards the origina) and the final tangent direction
        // (point from the via point towards the insertion).
        SimTK::UnitVec3 t_P =
                cable.calcViaPointInitialTangentDirection(state, ix);
        SimTK::UnitVec3 t_Q = cable.calcViaPointFinalTangentDirection(state, ix);
    ...
    // direction of the cable at insertion point, pointing away from the
    // insertion in the direction of the tension force.
    forceConsumer.consumePointForce(state,
