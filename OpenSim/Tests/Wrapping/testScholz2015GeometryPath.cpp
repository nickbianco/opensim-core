...
// A set of (coordinate value, moment arm) samples taken across a coordinate's
// range of motion.
typedef std::vector<std::pair<double, double>> DataPoints;

TEST_CASE("Warm start") {
    auto buildModel = [](bool warmStart, double defaultRz) {
        Model model;
        ...
        // Pin joint 'rz' about ground z; head hangs 1 m below the joint.
        auto* pin = new PinJoint("base_to_head",
                *base, SimTK::Vec3(0), SimTK::Vec3(0),
                *head, SimTK::Vec3(0, 1, 0), SimTK::Vec3(0));
        auto& rz = pin->updCoordinate();
        rz.setName("rz");
        rz.setRangeMin(-0.5);
        rz.setRangeMax(0.5);
        rz.setDefaultValue(defaultRz);
        model.addJoint(pin);

        // Ellipsoid obstacle attached to the base (matches pendulum.osim).
        ...
        // Low-stiffness PathSpring carrying the Scholz path (stable dynamics).
        auto* spring = new PathSpring();
        spring->setName("path_spring");
        spring->setRestingLength(0.0);
        spring->setDissipation(0.5);
        spring->setStiffness(2.0);
        spring->set_path(Scholz2015GeometryPath());
        model.addComponent(spring);

        Scholz2015GeometryPath& path =
                spring->updPath<Scholz2015GeometryPath>();
        path.appendPathPoint(*head, SimTK::Vec3(0));
        path.appendObstacle(*ellipsoid,
                SimTK::Vec3(0.10125963389873505, 0, 0));
        path.appendPathPoint(*base, SimTK::Vec3(0));
        path.setUseWarmStart(warmStart);
        return model;
    };

    auto computeDataPoints = [](const Model& model, std::string coordinatePath,
            std::string musclePath) -> DataPoints {
    ...
    auto compareDataPoints = [](const DataPoints& lhs, const DataPoints& rhs,
        ...
        CHECK_THAT(error.norm(), Catch::Matchers::WithinAbs(0.0, tolerance));
        CAPTURE(error);
    };

    Model model("pendulum.osim");
    model.initSystem();
    // model.updComponent<Coordinate>("/jointset/base_to_head/rz")
    //      .setDefaultValue(0.0);

    auto data_points = computeDataPoints(model,
            "/jointset/base_to_head/rz",
            "/forceset/scholz_muscle");
...
// Diagnostic: isolate the moment arm at a SINGLE configuration reached from
// different default values, to test whether the default (i.e., the initial path
// solution) biases the result and whether the warm-start flag changes anything.
TEST_CASE("Warm start investigation") {
    auto momentArmAt = [](double defaultRz, double targetRz, bool warmStart) {
        Model model("pendulum.osim");
        auto& path = model.updComponent<Scholz2015GeometryPath>(
                "/forceset/scholz_muscle/path");
        path.setUseWarmStart(warmStart);
        auto& coord =
                model.updComponent<Coordinate>("/jointset/base_to_head/rz");
        coord.setDefaultValue(defaultRz);
        SimTK::State state = model.initSystem();
        const auto& muscle = model.getComponent<Muscle>(
                "/forceset/scholz_muscle");
        coord.setValue(state, targetRz);
        model.equilibrateMuscles(state);
        model.realizeReport(state);
        return muscle.getPath().computeMomentArm(state, coord);
    };

    const double target = 0.0;
    for (bool ws : {false, true}) {
        std::cout << "warmStart=" << ws << " momentArm at rz=" << target
                  << " reached from:\n"
                  << "  default= 0.00 (init at target) -> "
                  << momentArmAt(0.0, target, ws) << "\n"
                  << "  default=-0.50                  -> "
                  << momentArmAt(-0.5, target, ws) << "\n"
                  << "  default= 0.45                  -> "
                  << momentArmAt(0.45, target, ws) << "\n";
    }

    // Workaround check: build the moment-arm curve by evaluating each point on a
    // FRESH state (one setValue + realize per point) instead of sweeping a
    // single reused state. This should be independent of the default value.
    auto freshCurve = [&](double defaultRz) {
        Model model("pendulum.osim");
        model.updComponent<Coordinate>("/jointset/base_to_head/rz")
                .setDefaultValue(defaultRz);
        model.initSystem();
        const auto& coord =
                model.getComponent<Coordinate>("/jointset/base_to_head/rz");
        SimTK::Vector x = createVectorLinspace(41,
                coord.getRangeMin(), coord.getRangeMax());
        DataPoints dp;
        for (int i = 0; i < x.size(); ++i) {
            // Fresh state per point: no history is carried between samples.
            SimTK::State state = model.initSystem();
            const_cast<Coordinate&>(coord).setValue(state, x[i]);
            model.equilibrateMuscles(state);
            model.realizeReport(state);
            const auto& muscle = model.getComponent<Muscle>(
                    "/forceset/scholz_muscle");
            dp.emplace_back(x[i], muscle.getPath().computeMomentArm(state, coord));
        }
        return dp;
    };

    DataPoints freshLow = freshCurve(-0.5);
    DataPoints freshHigh = freshCurve(0.0);
    double freshMaxDiff = 0.0;
    for (size_t i = 0; i < freshLow.size(); ++i) {
        freshMaxDiff = std::max(freshMaxDiff,
                std::abs(freshLow[i].second - freshHigh[i].second));
    }
    std::cout << "FRESH-per-point curve maxDiff (default -0.5 vs 0.0) = "
              << freshMaxDiff << "\n";

    // Reused-state sweep: one state carried across the whole range of motion
    // (the scenario that originally exposed the inconsistency). With warm
    // starts disabled the curve should now be independent of the default value;
    // with warm starts enabled the history dependence may remain.
    auto reusedCurve = [&](double defaultRz, bool warmStart) {
        Model model("pendulum.osim");
        model.updComponent<Scholz2015GeometryPath>(
                "/forceset/scholz_muscle/path").setUseWarmStart(warmStart);
        model.updComponent<Coordinate>("/jointset/base_to_head/rz")
                .setDefaultValue(defaultRz);
        SimTK::State state = model.initSystem();
        const auto& coord =
        ...
        const auto& muscle = model.getComponent<Muscle>(
                "/forceset/scholz_muscle");
        SimTK::Vector x = createVectorLinspace(202,
                coord.getRangeMin(), coord.getRangeMax());
        DataPoints dp;
        for (int i = 0; i < x.size(); ++i) {
            coord.setValue(state, x[i]);
            model.equilibrateMuscles(state);
            model.realizeReport(state);
            dp.emplace_back(x[i], muscle.getPath().computeMomentArm(state, coord));
        }
    ...
    for (bool ws : {false, true}) {
        DataPoints low = reusedCurve(0.0, ws);
        DataPoints high = reusedCurve(0.45, ws);
        double sweepMaxDiff = 0.0;
        for (size_t i = 0; i < low.size(); ++i) {
            sweepMaxDiff = std::max(sweepMaxDiff,
                    std::abs(low[i].second - high[i].second));
        }
        std::cout << "REUSED-state sweep, warmStart=" << ws
                  << ": curve maxDiff (default 0.0 vs 0.45) = "
                  << sweepMaxDiff << "\n";
    }
}
